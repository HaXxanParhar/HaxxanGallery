...
import com.drudotstech.customgallery.crop.CropImageView;
...
public class AlbumPicturesActivity extends AppCompatActivity implements
        OnPictureSelectedCallBack, PicturesAdapter.LongClickCallback, PicturesAdapter.PictureClickCallback {

    private final Context context = AlbumPicturesActivity.this;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (list.isEmpty()) {
            ...
            adapter = new PicturesAdapter(list, AlbumPicturesActivity.this, this, this);
    ...
    public ArrayList<GalleryMediaModel> getAllImagesByFolder(String albumPath) {
        ...
        Cursor cursor = AlbumPicturesActivity.this.getContentResolver().
                query(contentUri,
    ...
    public void onPictureClicked(PicturesAdapter.PicHolder holder, int position) {
        ...
        if (!isSelectionEnabled) {
//            openImageInFragment(holder, position, list);
            openImageCropper(holder, position, list);
        } else {
            setSelected(holder, position, list);
        }
    }

    private void openImageCropper(PicturesAdapter.PicHolder holder, int position, ArrayList<GalleryMediaModel> pics) {
        File file = new File(pics.get(position).getMediaPath());
...
//                .setRequestedSize(400, 400)
                .setCropMenuCropButtonIcon(R.drawable.ic_baseline_navigate_next_24)
                .start(this);
    }
...
//     handle result of CropImageActivity
    ...
    private void openImageInFragment(PicturesAdapter.PicHolder holder, int position, ArrayList<GalleryMediaModel> pics) {
        ...
        PictureBrowserFragment browser = PictureBrowserFragment.newInstance(pics, position,
                AlbumPicturesActivity.this, this);
