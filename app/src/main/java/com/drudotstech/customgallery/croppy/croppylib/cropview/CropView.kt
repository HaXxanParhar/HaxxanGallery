...
import com.drudotstech.customgallery.croppy.aspectratiorecyclerviewlib.aspectratio.model.AspectRatio.ASPECT_FREE
import com.drudotstech.customgallery.croppy.croppylib.cropview.AspectMode.*
...
import com.drudotstech.customgallery.croppy.croppylib.util.extensions.*
import com.drudotstech.customgallery.croppy.croppylib.util.model.AnimatableRectF
import com.drudotstech.customgallery.croppy.croppylib.util.model.Corner
import com.drudotstech.customgallery.croppy.croppylib.util.model.Corner.*
import com.drudotstech.customgallery.croppy.croppylib.util.model.Corner.NONE
import com.drudotstech.customgallery.croppy.croppylib.util.model.DraggingState
import com.drudotstech.customgallery.croppy.croppylib.util.model.DraggingState.DraggingCorner
import com.drudotstech.customgallery.croppy.croppylib.util.model.DraggingState.DraggingEdge
...
class CropView @JvmOverloads constructor(
    ...
    /**
     * Blurred bitmap that will show in non selected area
     */
...
//        // Rounded image path
//        val w: Float = borderPaint.strokeWidth
...
//        canvas?.drawColor(maskBackgroundColor) // draw background color
        blurredBitmap?.let { canvas?.drawBitmap(it, bitmapMatrix, emptyPaint) }
        canvas?.restore()
...
//        drawCornerToggles(canvas)
    ...
    /**
     * Set blurred bitmap that will be used to show blurred part of the image.
     * And invalidate to show.
     */
    ...
    fun setAspectRatio(aspectRatio: AspectRatio) {
        ...
        aspectAspectMode = when (aspectRatio) {
            ASPECT_FREE -> FREE
            AspectRatio.ASPECT_FULLSCREEN -> FULLSCREEN
            else -> ASPECT
        }
    ...
    private fun initialize() {
        ...
        // uncomment to add the margin around the crop
...
//        viewHeight = measuredHeight.toFloat() - (marginInPixelSize * 2)
    ...
    private fun initializeBitmapMatrix() {
        // haxxan : here is the height width of the original bitmap
    ...
    private fun onCornerPositionChanged(corner: Corner, motionEvent: MotionEvent) {
        when (aspectAspectMode) {
            ...
            FULLSCREEN -> {
                when (corner) {
                    TOP_RIGHT -> {
                        cropRect.setTop(viewRect.top)
                        cropRect.setRight(viewRect.right)
                    }
                    TOP_LEFT -> {
                        cropRect.setTop(viewRect.top)
                        cropRect.setLeft(viewRect.left)
                    }
                    BOTTOM_RIGHT -> {
                        cropRect.setBottom(viewRect.bottom)
                        cropRect.setRight(viewRect.right)
                    }
    ...
    private fun onEdgePositionChanged(edge: Edge, motionEvent: MotionEvent) {
        ...
        when (aspectAspectMode) {
            ...
            FULLSCREEN -> {
                when (edge) {
                    LEFT -> cropRect.setLeft(viewRect.left)
                    TOP -> cropRect.setTop(viewRect.top)
                    RIGHT -> cropRect.setRight(viewRect.right)
                    BOTTOM -> cropRect.setBottom(viewRect.bottom)
                    else -> return
                }
            }
    ...
    private fun calculateMinRect() {
        ...
        when (aspectAspectMode) {
            ...
            FULLSCREEN -> {
                val scaleWidth = minSize / viewRect.width()
                val scaleHeight = minSize / viewRect.height()
